...
We illustrate three key features of the proposed framework with computer simulations 
All simulations are based on a simple python implementation of the network, available at {cite:t}`https://github.com/tspisak/fep-attractor-networks`. The implementation favors clarity over efficiency - it implements both $\sigma$ and boundary states as separate classes, and is not optimized for performance.
...
The crucial role of attractor dynamics in elucidating brain function, particularly through the lens of self-organisation, mandates a foundational question: what type of attractor network naturally arises from the first principles of such self-organisation, as articulated by the Free Energy Principle (FEP) {cite:p}`https://doi.org/10.1016/j.physrep.2023.07.001; https://doi.org/10.1038/nrn2787`? This manuscript addressed this question. We demonstrated mathematically, by recourse to a prototypical parametrisation, that the ensuing networks manifest as non-equilibrium steady-state (NESS) systems {cite:p}`https://doi.org/10.1088/1751-8113/43/37/375003; 10.1088/0305-4470/37/3/L01` that have a stationary state probability distribution governed by the symmetric component of their synaptic efficacies, conforming to a Boltzmann-like form {cite:p}`https://doi.org/10.1017/CBO9780511623257; https://doi.org/10.1162/neco.1997.9.8.1735`. This renders the resulting self-organizing attractor networks a generalisation of canonical Boltzmann machines or stochastic Hopfield networks {cite:p}`https://doi.org/10.1162/089976602760128018; 10.1073/pnas.79.8.2554`, but distinguished by their capacity for asymmetric coupling and continuous-valued neuronal states. The assumptions underpinning this derivation - namely, the existence of a (deep) particular partition and the ensuing imperative to minimise variational free energy - are not merely parsimonious but arguably fundamental for any random dynamical system that maintains its integrity despite a changing environment {cite:p}`https://doi.org/10.1016/j.plrev.2023.08.016`.

Our formulation reveals that the minimisation of variational free energy at the micro-scale, i.e., by individual network nodes ("subparticles"), gives rise to a dual dynamic within the active inference framework {cite:p}`https://doi.org/10.1016/j.tics.2009.04.005`. Firstly, it prescribes Bayesian update dynamics for the individual network nodes, homologous to the stochastic relaxation observed in Boltzmann architectures (e.g. stochastic Hopfield networks), with high neuroscientif relevance, especially for coarse-grained brain networks {cite:p}`doi:10.1038/nn.4406; doi:10.1016/j.neuroimage.2023.120300; https://doi.org/10.48550/arXiv.2402.02191`. Secondly, it engenders a distinctive synaptic plasticity - a local, incremental learning rule - that continuously adjusts coupling weights to preserve low free energy in anticipation of future sensory encounters, effectively implementing action selection in the active inference sense {cite:p}`https://doi.org/10.1016/j.neubiorev.2016.06.022`. The learning rule itself displays a high neurobiological plausibility, as it resembles both generalized Hebbian-anti-Hebbian learning {cite:p}`https://doi.org/10.1016/0893-6080(89)90044-0` and predictive coding {cite:p}`https://doi.org/10.1016/j.neuron.2005.02.001` by contrasting the current correlations of inputs with that predicted by the network's generative model. However, unlike these frameworks, the derived learning rule was not designed to mimic known neurobiological processes, but rather derived from first principles of self-organisation.

As motivated mathematically and demonstrated in silico, a pivotal outcome of the FEP-driven simultaneous learning and inference process is the propensity of the network to establish *approximately orthogonal attractor states*. This isn't just a side effect; it's an unavoidable result of reducing free energy; which entails minimizing model complexity and maximizing accuracy simultaneously. Thus, free-energy minimizing attractor networks naturally approximate one of the most efficient attractor network architectures, the projection networks articulated by Kanter and Sompolinsky {cite:p}`10.1103/physreva.35.380`.
...
Indeed, empirical investigations are beginning to furnish evidence for attractor dynamics in large-scale brain activity {cite:p}`https://doi.org/10.1002/wcs.1; https://doi.org/10.1098/rstb.2011.0351; https://doi.org/10.1007/978-3-642-96469-5; https://doi.org/10.1038/nn.4497; https://doi.org/10.1523/JNEUROSCI.2523-11.2012; https://doi.org/10.1098/rstb.2011.0351; https://doi.org/10.1016/j.neunet.2023.11.027; https://doi.org/10.1016/j.neuroimage.2022.119595`. In a recent work - using an attractor network model very similar to the hereein presented -  we have demonstrated that standard resting-state networks (RSNs) are manifestations of brain attractors that can be reconstructed from fMRI brain connectivity data {cite:p}`10.1101/2023.11.03.565516`. Most importantly, these empirically reconstructed large-scale brain attractors were found to be largely orthogonal, a key feature of the self-organizing attractor networks described here. Future research needs to carefully check if these attractors, and the computing abilities that come with them, are truly an unavoidable result of system activity that minimizes free energy. 

Besides neuroscientific relevance, our work has also manifold implications for artificial intelligence. Firstly, energy-efficient neuromorphic architectures {cite:p}`https://doi.org/10.1038/s43588-021-00184-y` might harness both the intrinsic stochasticity and stability of attractor dynamics for robust in-memory computation, particularly within the emerging field of thermodynamic computing {cite:p}`https://doi.org/10.1038/s41467-025-59011-x`. Secondly, within our framework, learning and inference are not disparate processes but rather two complementary facets of variational free energy minimisation through active inference {cite:p}. As we demonstrated with simulations, this unification naturally endows the proposed architecture with characteristics of continual or lifelong learning {cite:p}`https://doi.org/10.48550/arXiv.2302.00487` - the ability for machines to gather data and fine-tune its internal represnetation continously during functioning - and an inherent resilience to catastrophic forgetting {cite:p}`https://doi.org/10.48550/arXiv.2312.10549` - the tendency of current deep learning architectures to loose old representations when learning new ones. 
Thirdly, because it's guided by the FEP, the resulting system will - by design - strike a balance between accuracy and complexity and therefore expected to generalize well to new data. Further, our active inference-based attractor network is expected to show information-seeking behaviors (curiosity) {cite:p}`https://doi.org/10.1162/neco_a_00999`, and provide a basis for investigating qualities linked to sentience {cite:p}`https://doi.org/10.1016/j.biopsycho.2023.108741`, as a consequence of the network's implicit use of internal models to predict, infer, and direct action.
