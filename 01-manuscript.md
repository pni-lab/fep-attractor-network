...
The Markov blanket structure defines a graphical model where each $\sigma_i$ is a node and edges represent pairwise interactions mediated by the boundary states $s_{ij}, a_{ij}$. After marginalizing out the boundary states, we obtain direct interactions between the $\sigma$ nodes with potentially asymmetric couplings $J_{ij}$.

To construct a joint distribution over all nodes, we need to find a distribution $P(\bm{\sigma})$ that is consistent with all the pairwise conditionals $P(\sigma_i|\sigma_j)$ we have derived. This consistency requirement - that marginalizing the joint distribution must recover each specified conditional - places strong mathematical constraints on the possible forms of $P(\bm{\sigma})$.

When we attempt to construct such a joint distribution by combining the pairwise interaction terms, we obtain:
...
So far our derivation only relied on the sparsely coupled structure of the system (deep particular partition), but did not utilize the free energy principle itself. We now consider the implications of free energy minimization on the dynamics of the derived recurrent neural network. We start by writing up variational free energy (VFE, eq. [](#eq-free-energy-functional)) from the point of view of a single node of the attractor network $\sigma_i$, given observations from all other nodes $\sigma_{\backslash i}$:
...
Besides neuroscientific relevance, our work has also manifold implications for artificial intelligence research. Firstly, energy-efficient neuromorphic architectures {cite:p}`https://doi.org/10.1038/s43588-021-00184-y` might harness both the intrinsic stochasticity in our framework and the stability of attractor dynamics for robust in-memory computation, particularly within the emerging field of thermodynamic computing {cite:p}`https://doi.org/10.1038/s41467-025-59011-x`. Secondly, within our framework, learning and inference are not disparate processes but rather two complementary facets of variational free energy minimization through active inference {cite:p}. As we demonstrated with simulations, this unification naturally endows the proposed architecture with characteristics of continual or lifelong learning - the ability for machines to gather data and fine-tune its internal representation continuously during functioning {cite:p}`https://doi.org/10.48550/arXiv.2302.00487`. Thirdly, the proposed network architecture may naturally overcome catastrophic forgetting {cite:p}`https://doi.org/10.48550/arXiv.2312.10549` - the tendency of current deep learning architectures to lose old representations when learning new ones - by spontaneously "replaying" its own attractors (or sequences thereof), even if no external input is introduced (zero bias). This phenomenon can also serve as a model for the spontaneous fluctuations observed during resting state brain activity (also related to "day-dreaming") and holds promise for taking advantage of similar mechanisms in artificial systems. To examine how these properties of FEP-based attractor networks scale to more complex, diverse, and extended learning scenarios is a promising direction for further studies.
